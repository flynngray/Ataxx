...
class Board {
    ...
    private void setupColorVariables() {
        int redPieces = 0;
        int bluePieces = 0;
    ...
    void clear() {
        ...
        for (int i = 0; i < colNames.length; i++) {
            for (int k = 0; k < rowNames.length; k++) {
                unrecordedSet(colNames[i], rowNames[k], EMPTY);
            }
        }
        for (int i = 0; i < _board.length; i++) {
            if (get(i) != EMPTY) {
    ...
    boolean legalMove(Move move) {
        ...
        if (canMove(whoseMove()) && move.isPass()) {
            return false;
        }
        if (!move.isPass()) {
            int colDiff = abs(move.col1() - move.col0());
            int rowDiff = abs(move.row1() - move.row0());
            if (!(colDiff < 3 && rowDiff < 3)) {
                return false;
            }
            if (get(move.col0(), move.row0()) != whoseMove()) {
                return false;
            }
            if (move.col1() > 'g' || move.col1() < 'a') {
                return false;
            }
            if (move.row1() > '7' || move.row1() < '1') {
                return false;
            }
    ...
    boolean legalMove(char c0, char r0, char c1, char r1) {
        int colDiff = abs(c1 - c0);
        int rowDiff = abs(r1 - r0);
        if (!(colDiff < 3 && rowDiff < 3)) {
            return false;
        }
    ...
    void makeMove(Move move) {
        ...
        set(move.col1(), move.row1(), get(move.col0(), move.row0()));
        _numMoves++;
        int incrPieces = 1;
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                if (get(neighbor(index
                        (move.col1(), move.row1()), i, j)) == opponent) {
                    set(neighbor(index
                            (move.col1(), move.row1()), i, j), _whoseMove);
                    incrPieces++;
        ...
        _whoseMove = opponent;
        incrPieces(_whoseMove, incrPieces);
        seeifGameShouldEnd();
        announce();
    }

    private void seeifGameShouldEnd() {
        setupColorVariables();
    ...
    ArrayList<Move> generateLegalMoves() {
        ArrayList<Move> validMoves = new ArrayList<Move>();
        for (int col0 = 'a'; col0 <= 'g'; col0++) {
            for (int row0 = '1'; row0 <= '7'; row0++) {
                if (get((char) col0, (char) row0) == whoseMove()) {
                    for (int k = -2; k < 3; k++) {
                        for (int j = -2; j < 3; j++) {
                            Move currMove = Move.move((char) col0, (char) row0,
                                    (char) (col0 + k), (char) (row0 + j));
    ...
    void undo() {
        ...
        while (_undoSquares.peek() != null && !_undoSquares.empty()) {
            _board[_undoSquares.pop()] = _undoPieces.pop();
        }
        _undoSquares.pop();
        _numMoves--;
        setupColorVariables();
    ...
    void setBlock(char c, char r) {
        ...
        int coldiff = c - 'a';
        char oppcol = (char) ('g' - coldiff);
        int rowdiff = r - '1';
        char opprow = (char) ('7' - rowdiff);

        set(c, r, BLOCKED);
        set(oppcol, r, BLOCKED);
        set(c, opprow, BLOCKED);
        set(oppcol, opprow, BLOCKED);
        _totalOpen -= 4;
    ...
    /** number of moves that have passed. */
    private int _numMoves;

    /** names of the rows. */
    private char[] rowNames = {'1', '2', '3', '4', '5', '6', '7'};

    /** names of the columns. */
    private char[] colNames = {'a', 'b', 'c', 'd', 'e', 'f', 'g'};
}
